...
function find_info() {
	...
	if (document.find.phone.value == "") {
		...
		var fid = document.find.id.value;
		var fname = document.find.name.value;
		var fphone = document.find.phone.value;
		// 새 창 열기("이동할 위치(페이지)", "창 이름", "창 크기", "옵션")
		wr = window.open("./PwFindChk.mb?fid="+fid+"&fname="+fname+"&fphone="+fphone, "", "width=300, height=250");
		wr.moveTo(x,y);
}

function fun_cancel() {
	var con = confirm("찾기를 취소하시겠습니까?");
...
<body>
	...
	<div id="wrap_conts" style="text-align: center">
		<div id="conts">
			<h2 id="tit">비밀번호 찾기</h2>
				<form action="./main.me" method="get" id="find" name="find">
					<div>
						아이디　 :
						<input type="text" id="id" name="id" placeholder="ex) abc@naver.com" size="50"> <br><br>
						이　　름 :
